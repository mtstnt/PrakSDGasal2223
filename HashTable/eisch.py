...
class EischHashTable:
  ...
  def insert(self, item: any) -> None:
    ...
    #  lihat dari index terakhir, maju ke index 0 sampai ketemu tempat kosong.
    #  Tempat kosong dilink dengan index awal (jadi index kedua)
    else:
      origin = self.table[idx]
      for i in range(self.capacity - 1, -1, -1):
        if self.table[i] is None:
          self.table[i] = Node(item)
          self.table[i].next = origin.next
          origin.next = i
          break
